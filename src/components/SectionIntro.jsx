export default function SectionIntro({ label, title, copy, align = 'left' }) {
  ...
  return (
    <div className={centered ? 'mx-auto max-w-4xl text-center' : 'max-w-3xl'}>
      <div className={`mb-6 flex items-center gap-3 text-[0.72rem] font-medium uppercase editorial-kicker text-warmbrown ${centered ? 'justify-center' : ''}`}>
      ...
      <h2 className="font-display text-[2.45rem] leading-[0.96] text-ink sm:text-[3.2rem] md:text-[3.75rem]">
        {title}
      </h2>
      {copy ? (
        <p className={`mt-6 text-[1rem] leading-8 text-ink/64 sm:text-[1.07rem] ${centered ? 'mx-auto max-w-2xl' : 'max-w-2xl'}`}>
          {copy}
        </p>
      ) : null}
    </div>
